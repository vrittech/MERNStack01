...
const MessageController = {
  createMessage: async (req: Request, res: Response) => {
    ...
    try {
      ...
      const conversation = await conversationService.getConversationById({
        _id: createdMessage.conversation_id.toString(),
      });

      const members = conversation?.members;
      members?.map((member) => {
        if (member.toString() !== from_id.toString()) {
          const client = socketService.getClient(member.toString());
         
         client?.socket.send(JSON.stringify(createdMessage))
  ...
  getMessagesOfConversation: async (req: Request, res: Response) => {
    ...
    try {
      ...
      const messages = await messageService.getMessagesofConverstaion(
        conversation_id
      );
      return res.status(200).json({
